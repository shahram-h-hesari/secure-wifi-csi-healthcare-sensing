...
def generate_normal_activity_csi(length=200, noise_level=0.05, random_state=None):
    """
...
generate_normal_signal = generate_normal_activity_csi


def generate_fall_event_csi(length=200, noise_level=0.05, random_state=None):
    """
    Generate a synthetic CSI-like signal representing a fall-like event.

    A fall-like event is modeled as a sudden transient spike followed by a
    short recovery dip. This is a simplified conceptual model of an abrupt
    ...
    signal = generate_normal_activity_csi(
        length=length,
    ...
    width = rng.integers(
        low=max(3, int(length * 0.025)),
        high=max(4, int(length * 0.07))
    )
...
# Alias for backward compatibility
generate_fall_like_signal = generate_fall_event_csi
...
def generate_synthetic_dataset(
    ...
    y : numpy.ndarray of shape (2 * n_samples_per_class,)
        Class labels. 0 = normal activity, 1 = fall-like event.

    Notes
    -----
    This dataset is synthetic and should be used only to demonstrate the
    signal-processing and machine learning workflow. It does not represent
    real WiFi CSI measurements or validated fall-detection data.
    """
...
# Alias for backward compatibility
generate_dataset = generate_synthetic_dataset
